...
import { createProduct, productImageSlider, initImgSwiper, calculateDisscount,openProductPage } from "./module.js";
...
let minInputValue = 0, maxInputValue = 590;
...
async function getProducts() {
    ...
    try {
        ...
        document.title = category[0].toUpperCase() + category.substring(1) + " Products";
        document.getElementById(category).checked = true;
        const limit = 4;

        let products;
        if (category !== "all") {
            const { data } = await axios.get(`https://klbtheme.com/fynode/wp-json/wc/store/products?category=${category}&per_page=${limit}&page=${page}`);
            products = data;

        } else {
            const { data } = await axios.get(`https://klbtheme.com/fynode/wp-json/wc/store/products?per_page=${limit}&page=${page}`);
            products = data;
...
function customPagination(numOfPages, page, category) {
    ...
    let pagination = `<li class = "d-flex justify-content-center align-items-center"><a class = "text-decoration-none text-dark" href = "./shop.html?category=${category}&page=${page - 1}"><i class="fa-solid fa-angle-left"></i></a></li>`;
    ...
    pagination += `<li class = "d-flex justify-content-center align-items-center"><a class = "text-decoration-none text-dark" href = "./shop.html?category=${category}&page=${page + 1}"><i class="fa-solid fa-angle-right"></i></a></li>`;
...
function createProducts(products) {
    ...
    document.querySelector(".shop-content .row").innerHTML = products.length !== 0 ? products.map(product => {
        return customizeProduct(product);
    }).join("") : createNoProductsMessage();
}
...
function filterByCategory() {
    ...
    checks.forEach(input => {
        input.onchange = (e) => {
            if (e.target.checked)
                window.location.href = `./shop.html?category=${e.target.value}&page=${1}`;
        }
    })
}
